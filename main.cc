...
void usage() {
  printf("usage: $main [thread_cnt] [table_cnt] [insert_cnt]\n");
  printf("\tthread_cnt [1, 100], default 1\n");
  printf("\table_cnt [1, 1000], default 100\n");
  printf("\tinsert_cnt [100, 10M], default 1000\n");
  exit(-1);
}
...
bool parse_args(int argc, char* argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (i == 3) {
      insert_cnt = stol(argv[3]);
      if (insert_cnt == 0 || insert_cnt < 100 || insert_cnt > 10 * 1000 * 1000) {
        usage();
        return false;
      }
    }
